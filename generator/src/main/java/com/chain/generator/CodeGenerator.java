...
import com.chain.generator.config.*;
import com.chain.generator.config.query.SpringBootPlusMySqlQuery;
import com.chain.generator.config.query.SpringBootPlusSqlServerQuery;
import com.chain.generator.constant.GeneratorConstant;
import com.chain.generator.exception.GeneratorException;
...
public class CodeGenerator {
    ...
    public void generator(GeneratorProperties generatorProperties) {
        ...
        if (CollectionUtil.isEmpty(tableConfig)) {
    ...
    public void init(String tableName, GeneratorProperties generatorProperties) {
        ...
        if (StrUtil.isBlank(parentPackage)) {
            throw new GeneratorException("项目父包不能为空");
        }
        // 如果包路径为空，转换包名称路径
        if (StrUtil.isNotBlank(parentPackage)) {
        ...
        if (StrUtil.isBlank(mavenModuleName)) {
    ...
    public void generator(String tableName, String pkIdName, GeneratorProperties generatorProperties) {
        ...
        if (StrUtil.isBlank(globalConfig.getServiceName())) {
        ...
        if (CollectionUtil.isNotEmpty(tablePrefixList)) {
            strategyConfig.setTablePrefix(tablePrefixList.toArray(new String[]{}));
        }


        if (StrUtil.isBlank(strategyConfig.getSuperEntityClass())) {
            // 自定义继承的Entity类全称，带包名
            strategyConfig.setSuperEntityClass(GeneratorConstant.SUPER_ENTITY_CLASS);
        }
        if (StrUtil.isBlank(strategyConfig.getSuperControllerClass())) {
